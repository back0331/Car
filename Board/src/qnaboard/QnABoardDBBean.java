...
public class QnABoardDBBean {   
    ...
    public void insertArticle(QnABoardDataBean qna) throws Exception {
        ...
        try {
            ...
            // 쿼리를 작성
            sql = "insert into qnaboard(article_no,id,article_subject,password,reg_date,";
            sql+="article_content,ip,article_type,com_check) values(board_num.NEXTVAL,?,?,?,?,?,?,?,'n')";
    ...
    public int getArticleCount() throws Exception {
        ...
        try {
            ...
            pstmt = conn.prepareStatement("select count(*) from qnaboard");//전체 글 갯수
...
return articleList;
    ...
    public QnABoardDataBean getArticle(int article_no) throws Exception {
        ...
        try {
            ...
            pstmt = conn.prepareStatement(
            "select * from qnaboard where article_no = ?");
    ...
    pstmt = conn.prepareStatement(
            "update qnaboard set article_subject=?,article_content=? where article_no=?");
...
return r;
    ...
    public int getArticleCount(int n, String searchKeyword) throws Exception{
		...
		{
			conn = getConnection();
			pstmt = conn.prepareStatement("select count (*) from qnaboard where "+column_name[n]+" like '%"+searchKeyword+"%'");
