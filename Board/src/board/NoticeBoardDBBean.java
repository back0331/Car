...
public class NoticeBoardDBBean {   
    ...
    public void insertArticle(NoticeBoardDataBean article) throws Exception {
    	...
    	try{   	
            ...
            pstmt = conn.prepareStatement(
            		"insert into noticeboard(article_no,article_subject,reg_date,article_content) values(board_num.NEXTVAL,?,?,?)");
    ...
    public int getArticleCount() throws Exception {
        ...
        try {
            ...
            pstmt = conn.prepareStatement("select count (*) from noticeboard");//전체 글 갯수
    ...
    public NoticeBoardDataBean getArticle(int article_no) throws Exception {
        ...
        try {
            ...
            pstmt = conn.prepareStatement(
            "select * from noticeboard where article_no = ?");
    ...
    pstmt = conn.prepareStatement(
            "update noticeboard set article_subject=?,article_content=? where article_no=?");
    ...
    public int deleteArticle(int article_no) throws Exception {
        ...
        try {
            ...
            pstmt = conn.prepareStatement("delete from noticeboard where article_no= ?" );
                    pstmt.setInt(1, article_no);                                 
                 r=   pstmt.executeUpdate();
...
return r;
    ...
    public int getArticleCount(int n, String searchKeyword) throws Exception{
		...
		{
			conn = getConnection();
			pstmt = conn.prepareStatement("select count (*) from noticeboard where "+column_name[n]+" like '%"+searchKeyword+"%'");
    ...
    public List getArticles(int start, int end) throws Exception {
        ...
        try {
            ...
            pstmt = conn.prepareStatement(
            		   ...
            		   + "from noticeboard order by article_no desc) where r >= ? and r<=?");
	...
	String[] column_name = {"article_subject","article_content"};
		...
		try
		{
			...
			String sql = "select article_no,article_subject,reg_date,article_content,r "
					+"from (select article_no,article_subject,reg_date,article_content, rownum r "
					+"from noticeboard where "+column_name[n]+" like '%"+searchKeyword+"%') where r >= ? and r <= ?";
  ...
  pstmt = conn.prepareStatement("select password from qnaboard where article_no=?");
