...
function check_pwd(pwd,no,pageNum){
	...
	if(pwd==pwdv){
		location.href("deletePro.jsp?article_no="+no+"&pageNum="+pageNum);
	}else{
		alert("비밀번호가 맞지않습니다.");
	}
}
</script> -->
<BODY bgcolor="${bodyback_c }"><!-- onload="begin()"  -->
...
%>  --%>
  ...
  <TR height="30">
    <TD colspan="2" align="middle" bgcolor="${title_c }" >
      <INPUT type="submit" value="확인"> <%-- onclick="check_pwd('<%=review.getPassword()%>','<%=article_no%>','<%=pageNum%>')" --%>
