...
<tr height="50">  
	...
	<td align="center" width="300">
	 <a href="/Board/review_board/lock.do?article_no=${article.article_no}&pageNum=${currentPage}">
...
<c:if test="${count > 0}">
  <c:set var="pageCount" value="${count / pageSize + ( count % pageSize == 0 ? 0 : 1)}"/>
   ...
   <c:if test="${startPage > 5}">
        <a href="/Board/review_board/list.do?pageNum=${startPage - 5 }">[이전]</a>
   </c:if>

   <c:forEach var="i" begin="${startPage}" end="${endPage}">
       <a href="/Board/review_board/list.do?pageNum=${i}">[${i}]</a>
   </c:forEach>

   <c:if test="${endPage < pageCount}">
        <a href="/Board/review_board/list.do?pageNum=${startPage + 5}">[다음]</a>
   </c:if>
</c:if>
