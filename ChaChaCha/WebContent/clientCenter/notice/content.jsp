...
function delete_content(no){
	if(confirm("정말로 삭제하시겠습니까??")){
		location.href="/clientCenter/notice/deletePro.do?article_no="+no;
	}
}
...
<c:if test="${user_id=='admin'}">
  ...
  <input type="button" value="글수정" onclick="document.location.href='/clientCenter/notice/updateForm.do?article_no=${article.article_no}&pageNum=${pageNum}'">&nbsp;&nbsp;&nbsp;&nbsp;
  <input type="button" value="글삭제" onclick="delete_content('${article.article_no}')">
   &nbsp;&nbsp;&nbsp;&nbsp;
   <input type="button" value="글목록"onclick="document.location.href='/clientCenter/notice/list.do?pageNum=${pageNum}'">
    </td>
  </tr>
</c:if>  
<c:if test="${user_id !='admin'}">
